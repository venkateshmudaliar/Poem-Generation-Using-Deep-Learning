...
from utility_functions import init_weight, get_poemdata


class SimpleRNN:
    # dimension of word embedding, hidden layer size, vocab size
    def __init__(self, D, M, V):
        self.D = D  # dimensionality of word embedding
        self.M = M  # hidden layer size
        self.V = V  # vocabulary size

    #unsupervise, momentum, regulaziation, activation function
    def fit(self, X, learning_rate=1., mu=0.99, reg=1.0, activation=T.tanh, epochs=500, show_fig=False):
        ...
        # initial weights
        # create word embeddings with the matrix forms
        We = init_weight(V, D)
        Wx = init_weight(D, M)
        Wh = init_weight(M, M)
        # m size vector
        ...
        def recurrence(x_t, h_t1):
            ...
            # softmax of activation function
    ...
    def set(self, We, Wx, Wh, bh, h0, Wo, bo, activation):
        ...
        #pi is initial word distribution
    def generate(self, pi, word2idx):
        ...
        # initial word coming from PI dist vocab
        #printing the word
        X = [np.random.choice(V, p=pi)]
        print(idx2word[X[0]], end=" ")

        #generating 4 lines at a time
        # change this number to change number of lines
        while n_lines < 4:
            # print "X:", X
            #getting the new words using predictOP depending on PI and concatenate the words
...
def train_poem_model():
    sentences, word2idx = get_poemdata()
    #dimension, hidden layers
    rnn = SimpleRNN(30, 30, len(word2idx))
    rnn.fit(sentences, learning_rate=1e-4, show_fig=True, activation=T.nnet.relu, epochs=3000)
    rnn.save('venky_quotes_weights.npz')


def generate_poem():
    sentences, word2idx = get_poemdata()
    rnn = SimpleRNN.load('venky_quotes_weights.npz', T.nnet.relu)
